...
try:
    ...
    if not os.path.exists(f"./{settings['REPO'].split('/')[1]}"):
     subprocess.check_output(["gh", "repo",  "clone", "https://github.com/" + settings['REPO'], "--",  "--quiet"])
    subprocess.check_output(["git", "--git-dir", f"./{settings['REPO'].split('/')[1]}/.git", "--work-tree", f"./{settings['REPO'].split('/')[1]}", "pull", "--quiet"])
except (subprocess.CalledProcessError,FileNotFoundError): NAME=""
...
for tx in txs:
    ...
    try:
        ...
        if os.path.isfile(f"/tmp/{tx}{FILE}"): signed.append(tx)
    except subprocess.CalledProcessError: print(f"sig {tx} failed. please check the tx or your config")
...
if choice == "1":
    try:
      subprocess.check_output(["git", "--git-dir", f"./{settings['REPO'].split('/')[1]}/.git", "--work-tree", f"./{settings['REPO'].split('/')[1]}", "pull", "--quiet"])
      for tx in signed:
        subprocess.check_output(["cp", f"/tmp/{tx}{FILE}", f"./{settings['REPO'].split('/')[1]}/transactions/signatures/{tx}{FILE}"])
      subprocess.check_output(["git", "--git-dir", f"./{settings['REPO'].split('/')[1]}/.git", "--work-tree", f"./{settings['REPO'].split('/')[1]}",
      "add", "./transactions/signatures/*"])
      subprocess.check_output(["git", "--git-dir", f"./{settings['REPO'].split('/')[1]}/.git", "--work-tree", f"./{settings['REPO'].split('/')[1]}",
      "commit", "-m", f"{NAME} signed {' '.join(signed)}"])
      subprocess.check_output(["git", "--git-dir", f"./{settings['REPO'].split('/')[1]}/.git", "--work-tree", f"./{settings['REPO'].split('/')[1]}", "push", "--quiet"])
